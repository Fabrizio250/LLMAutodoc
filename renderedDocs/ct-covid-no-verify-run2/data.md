...
A REST API accessible on port 5000 accepts POST requests at `/predict` with an image and bounding box parameters, returning predictions [1][2][3]. The API also exposes `/models` for listing available models and `/docs` for API documentation [2]. A frontend application consumes the API, launched via npm [0].
...
The dataset (COVIDx-CT) is a publicly available dataset hosted on Kaggle [0]. A derived dataset, COVIDx-SeqCT, is obtained from the original COVIDx-CT dataset [0]. No URL or direct link is documented in the repository. The specific collection method, institutional origin, or partner information is not documented in the repository.
...
Raw data is preprocessed using `src/preprocessing.py` with a target image resize operation to 224×224 pixels, taking inputs from `data/raw` and outputting cleaned data to `data/ct` [0]. No further data cleaning operations (e.g., deduplication, normalization, missing value handling, or filtering criteria) are documented in the repository.
...
COVIDx-CT dataset is split into train (143,778 examples: 35,996 Normal, 25,496 Pneumonia, 82,286 COVID-19), validation (25,486 examples: 11,842 Normal, 7,400 Pneumonia, 6,244 COVID-19), and test (25,658 examples: 12,245 Normal, 7,395 Pneumonia, 6,018 COVID-19) [0]. Bounding box coordinates for lung regions are available per example [0]. The derived COVIDx-SeqCT dataset contains sequences of exactly 16 CT slices extracted uniformly from CT scans; scans with fewer than 16 slices are discarded [0]. COVIDx-SeqCT is split into train (2,390 examples: 486 Normal, 403 Pneumonia, 1,501 COVID-19), validation (430 examples: 172 Normal, 112 Pneumonia, 146 COVID-19), and test (408 examples: 174 Normal, 100 Pneumonia, 134 COVID-19) [1]. All examples are annotated with one of three labels: normal, pneumonia, or covid19 [1]. Sampling criteria beyond slice count filtering are not documented in the repository.
