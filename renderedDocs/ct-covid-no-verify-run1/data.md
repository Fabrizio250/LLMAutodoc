...
A derived dataset, COVIDx-SeqCT, is composed of sequences of exactly 16 CT slices extracted uniformly from CT scans; scans with fewer than 16 slices are discarded [0]. It contains 2,390 training, 430 validation, and 408 test examples, each annotated with the same three labels [0][1]. The task served by both datasets is classification of CT images or CT scan sequences into normal, pneumonia, or COVID-19 categories [2]. The motivation includes end-to-end CT scan classification and generation of attention-based explanations relevant to the medical domain [2]. The original source of COVIDx-SeqCT is the COVIDx-CT dataset [0].
...
A REST API accessible on port 5000 exposes a `/predict` endpoint that accepts an image file and bounding box coordinates and returns model predictions [1][2][3]. Requests are submitted via HTTP POST with multipart/form-data encoding [3]. A frontend application consumes the API, started separately via npm [0]. Additional endpoints include `/models` for listing available models and `/docs` for API documentation [2].
...
Each CT image (slice) is annotated with one of three labels: normal, pneumonia (non-COVID-19), or covid19 [0]. All CT slices belonging to a single CT scan share the same label, assigned by an expert evaluating the patient's health state considering all images in the sequence [2]. Annotations apply at both the image level and the sequence level [1]. The annotators, the annotation procedure, and the specific guidelines followed are not documented in the repository.
...
COVIDx-CT dataset is split into train (143,778 examples: 35,996 Normal, 25,496 Pneumonia, 82,286 COVID-19), validation (25,486 examples: 11,842 Normal, 7,400 Pneumonia, 6,244 COVID-19), and test (25,658 examples: 12,245 Normal, 7,395 Pneumonia, 6,018 COVID-19) [0]. Bounding box coordinates for lung regions are available per example [0]. COVIDx-SeqCT, derived from COVIDx-CT, contains sequences of exactly 16 CT slices extracted uniformly from each scan; scans with fewer than 16 slices are discarded [0]. COVIDx-SeqCT is split into train (2,390 examples: 486 Normal, 403 Pneumonia, 1,501 COVID-19), validation (430 examples: 172 Normal, 112 Pneumonia, 146 COVID-19), and test (408 examples: 174 Normal, 100 Pneumonia, 134 COVID-19) [1]. Sampling criteria beyond the 16-slice minimum threshold are not documented in the repository.
