...
A derived dataset, COVIDx-SeqCT, is composed of sequences of exactly 16 CT slices extracted uniformly from CT scans; scans with fewer than 16 slices are discarded [0]. It contains 2,390 training, 430 validation, and 408 test examples, each annotated with the same three labels [1]. The task served by both datasets is classification of CT images or CT scan sequences into normal, pneumonia, or COVID-19 categories [2]. The motivation includes end-to-end CT scan classification and the use of attention mechanisms to generate predictions explainable in a medical context [2]. Dataset source beyond Kaggle availability is not documented in the repository.
...
The dataset (COVIDx-CT) is a publicly available dataset hosted on Kaggle [0]. COVIDx-SeqCT is derived from COVIDx-CT by extracting sequences of 16 CT slices from the original scans [0]. No URL, DOI, or further provenance details (e.g., originating institutions, collection methodology, or partner information) are documented in the repository.
...
Raw CT images are resized to 224×224 pixels via `src/preprocessing.py`, taking inputs from `data/raw` and outputting cleaned data to `data/ct` [0]. Data augmentation is applied to training data during dataset instantiation [1]. No further data cleaning operations (e.g., denoising, normalization, artifact removal, duplicate removal) are documented in the repository.
...
The COVIDx-CT dataset is split into train (143,778 examples: 35,996 Normal, 25,496 Pneumonia, 82,286 COVID-19), validation (25,486 examples: 11,842 Normal, 7,400 Pneumonia, 6,244 COVID-19), and test (25,658 examples: 12,245 Normal, 7,395 Pneumonia, 6,018 COVID-19) [0]. The COVIDx-SeqCT dataset, derived from COVIDx-CT by extracting sequences of exactly 16 CT slices uniformly from each scan (scans with fewer than 16 slices are discarded), is split into train (2,390 examples: 486 Normal, 403 Pneumonia, 1,501 COVID-19), validation (430 examples: 172 Normal, 112 Pneumonia, 146 COVID-19), and test (408 examples: 174 Normal, 100 Pneumonia, 134 COVID-19) [0][1]. All examples are annotated with one of three labels: normal, pneumonia (non-COVID-19), or COVID-19 [1]. Sampling criteria for the original train/validation/test partitioning are not documented in the repository.
