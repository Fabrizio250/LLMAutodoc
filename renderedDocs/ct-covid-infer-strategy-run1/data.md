...
A derived dataset, COVIDx-SeqCT, is composed of sequences of exactly 16 CT slices extracted uniformly from CT scans; scans with fewer than 16 slices are discarded [0]. It contains 2,390 training, 430 validation, and 408 test examples, each carrying the same three-class annotation [0][1]. The task served by both datasets is classification of CT images and CT scans into normal, pneumonia, or COVID-19 categories, supporting end-to-end CT scan classification with attention-based explainability in a medical diagnostic context [2].
...
The dataset (COVIDx-CT) is a publicly available dataset hosted on Kaggle [0]. A derived dataset, COVIDx-SeqCT, is obtained from the original COVIDx-CT dataset [0]. No URL, partner name, collection method, or scraping details are documented in the repository.
...
Raw data is preprocessed using `src/preprocessing.py`, which resizes images to 224×224 pixels and outputs cleaned data to `data/ct` from the `data/raw` source [0]. Data augmentation is applied to training data during dataset instantiation [1]. No further data cleaning operations (e.g., deduplication, normalization, missing value handling, outlier removal) are documented in the repository.
...
Input images are preprocessed via crop and resize using bicubic interpolation [0]. Images are then converted to tensors, normalized, moved to the target device, and unsqueezed along the batch dimension [1]. Data augmentation is applied during training, including random horizontal and vertical flip, Gaussian blur with kernel size 7 and standard deviation sampled uniformly from [0.05, 2.0], random affine transformations comprising scaling (scale ∈ [0.9, 1.1]), rotation (degrees ∈ [−30, 30]), and translation (translation percentage ∈ [0.0, 0.1]), and random shear augmentation on the x-axis with rotation sampled uniformly from [−20, 20] [2].
...
Each CT image (slice) is annotated with one of three labels: normal, pneumonia (non-COVID-19), or covid19 [0]. All CT slices belonging to a single CT scan share the same label, assigned by an expert evaluating the patient's health state considering all images in the sequence [2]. Annotations apply at both the image level (COVIDx-CT dataset: 143,778 training, 25,486 validation, 25,658 test examples) [0] and the sequence level (COVIDx-SeqCT dataset) [1]. The identity of the annotators, the specific annotation guidelines or protocols followed, and any inter-annotator agreement procedures are not documented in the repository.
