...
The dataset was built for multi-class classification of lung CT images into three categories: normal, pneumonia, and COVID-19 [0]. The primary use case is screening CT images to determine whether a patient has a COVID-19 infection or non-COVID-19 pneumonia [0]. The task is framed as a deep learning classification problem, with particular emphasis on minimizing false-negatives, given the health risks associated with undetected infections [0].
...
The dataset (COVIDx-CT) is a publicly available dataset hosted on Kaggle [0]. A derived dataset, COVIDx-SeqCT, is obtained from the original COVIDx-CT dataset by extracting sequences of CT slices [0]. No URL or direct link is documented in the repository. The specific data collection method, institutional origin, or partner information is not documented in the repository.
...
Each CT image (slice) is annotated with one of three labels: normal, pneumonia (non-COVID-19), or covid19 [0]. All CT slices belonging to a single CT scan share the same label, assigned by an expert evaluating the patient's health state considering all images in the sequence [2]. The annotation covers both individual slices and full CT sequences (COVIDx-SeqCT) [1]. The annotator role is described only as "an expert" [2]; specific annotator identities, number of annotators, inter-annotator agreement procedures, and formal annotation guidelines are not documented in the repository.
...
COVIDx-CT dataset is split into train (143,778 examples: 35,996 Normal, 25,496 Pneumonia, 82,286 COVID-19), validation (25,486 examples: 11,842 Normal, 7,400 Pneumonia, 6,244 COVID-19), and test (25,658 examples: 12,245 Normal, 7,395 Pneumonia, 6,018 COVID-19) [0]. Bounding box coordinates for lung regions are available per example [0]. COVIDx-SeqCT, derived from the original dataset, uses sequences of exactly 16 CT slices extracted uniformly per scan; scans with fewer than 16 slices are discarded [0]. Its split is: train (2,390: 486 Normal, 403 Pneumonia, 1,501 COVID-19), validation (430: 172 Normal, 112 Pneumonia, 146 COVID-19), test (408: 174 Normal, 100 Pneumonia, 134 COVID-19) [1]. All examples are labelled normal, pneumonia, or covid19 [1]. Sampling criteria beyond the 16-slice minimum threshold are not documented in the repository.
