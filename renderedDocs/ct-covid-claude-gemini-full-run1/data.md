...
The COVIDx-CT dataset consists of CT images (individual slices) annotated with one of three labels: normal, non-COVID-19 pneumonia, and COVID-19 pneumonia [2]. It contains 143,778 training, 25,486 validation, and 25,658 test examples [2]. Bounding box coordinates for lung regions are also provided for each example [0]. The three classes are highly unbalanced [2]. The dataset is freely available on Kaggle [0]. The task served is classification of CT images into the three diagnostic categories [2].

A derived dataset, COVIDx-SeqCT, is composed of sequences of exactly 16 CT slices extracted uniformly from CT scans; scans with fewer than 16 slices are discarded [0]. It contains 2,390 training, 430 validation, and 408 test examples, annotated with the same three labels [0][1]. This dataset serves the task of classifying a full CT scan in an end-to-end fashion [2]. Attention mechanisms are used to generate explanations for predictions in the medical domain [2].
...
A REST API accepting requests on port 5000 exposes a `/predict` endpoint that receives an image and its bounding box and returns a prediction [2][3]. The API is started via `python src/api.py` [1]. A frontend application consumes the API, started via `npm start` from the `src/frontend` directory [0]. Additional endpoints include `/models` for listing available models and `/docs` for API documentation [2].
...
The dataset was built for screening CT images of the lungs to classify patients as having COVID-19 infection, non-COVID-19 pneumonia, or normal condition [0]. The primary use case is a multi-class classification task with three classes: normal, pneumonia, and COVID-19 [0]. The dataset supports deep learning approaches, including per-image classification producing multiple predictions for a single CT scan [0]. A key clinical requirement is minimizing false-negatives, as infected patients may spread infection and face health complications [0].
...
The dataset originates from a public source freely available on Kaggle [0]. The primary dataset used is COVIDx-CT, containing CT scan images with bounding box coordinates for lung regions [0]. A derived dataset, COVIDx-SeqCT, is obtained from the original COVIDx-CT dataset [0]. No links, partner information, scraping details, or internal collection methodology are documented in the repository.
...
Input images are preprocessed using crop and resize with bicubic interpolation [0]. Images are then converted to tensors, normalized, moved to the target device, and unsqueezed along the batch dimension [1]. Data augmentation is applied during training, including random horizontal and vertical flip, Gaussian blur with kernel size 7 and standard deviation sampled uniformly from [0.05, 2.0], random affine transformations comprising scaling (scale ∈ [0.9, 1.1]), rotation (degrees ∈ [−30, 30]), and translation (translation percentage ∈ [0.0, 0.1]), as well as random shear augmentation on the x-axis with rotation sampled uniformly from [−20, 20] [2].
...
Each CT image (slice) is annotated with one of three labels: normal, pneumonia (non-COVID-19), or covid19 [0]. All CT slices belonging to a single CT scan share the same label, meaning annotation is performed at the scan (patient) level, with a patient's health state evaluated by an expert considering all images in the sequence [2]. The annotated dataset used is COVIDx-CT, comprising 143,778 training, 25,486 validation, and 25,658 testing examples [0]. A derived sequential dataset, COVIDx-SeqCT, applies the same three-label scheme across 2,390 training, 430 validation, and 408 test sequences [1]. The identity of the annotators, the specific annotation guidelines or protocols followed, and any inter-annotator agreement procedures are not documented in the repository.
...
COVIDx-CT dataset is split into train (143,778 examples: 35,996 Normal, 25,496 Pneumonia, 82,286 COVID-19), validation (25,486 examples: 11,842 Normal, 7,400 Pneumonia, 6,244 COVID-19), and test (25,658 examples: 12,245 Normal, 7,395 Pneumonia, 6,018 COVID-19) [0]. Bounding box coordinates for lung regions are available per example [0]. COVIDx-SeqCT, derived from the original dataset, uses sequences of exactly 16 CT slices extracted uniformly per scan; scans with fewer than 16 slices are discarded [0]. Its split is: train (2,390 examples: 486 Normal, 403 Pneumonia, 1,501 COVID-19), validation (430 examples: 172 Normal, 112 Pneumonia, 146 COVID-19), and test (408 examples: 174 Normal, 100 Pneumonia, 134 COVID-19) [1]. Both datasets use three classes: Normal, Pneumonia (non-COVID-19), and COVID-19 [1]. Sampling criteria beyond slice-count filtering are not documented in the repository.
